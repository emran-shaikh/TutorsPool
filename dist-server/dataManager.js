...
class DataManager {
    ...
    rejectUser(userId, rejectedBy, reason) {
        const userIndex = this.data.users.findIndex(u => u.id === userId);
        if (userIndex !== -1) {
            this.data.users[userIndex] = {
                ...this.data.users[userIndex],
    ...
    updateLearningGoal(studentId, goalId, updates) {
        // Initialize learningProgress if it doesn't exist
        if (!this.data.learningProgress) {
            this.data.learningProgress = {};
        }
    ...
    deleteLearningGoal(studentId, goalId) {
        // Initialize learningProgress if it doesn't exist
        if (!this.data.learningProgress) {
            this.data.learningProgress = {};
        }
    ...
    addUpcomingSession(studentId, session) {
        // Initialize learningProgress if it doesn't exist
        if (!this.data.learningProgress) {
            this.data.learningProgress = {};
        }
    ...
    completeSession(studentId, sessionId) {
        // Initialize learningProgress if it doesn't exist
        if (!this.data.learningProgress) {
            this.data.learningProgress = {};
        }
    ...
    updateReviewStatus(reviewId, status, adminId) {
    ...
    getNotificationsByUserId(userId, unreadOnly = false) {
    ...
    addBookingWithNotification(booking) {
        this.addBooking(booking);
        const newBooking = this.getAllBookings().find(b => b.studentId === booking.studentId &&
    ...
    updateBookingWithNotification(bookingId, updates, updatedBy) {
    ...
    updateTutorRatingStats(tutorId) {
    ...
    // Admin methods
    updateUserStatus(userId, status, adminId, reason) {
        const userIndex = this.data.users.findIndex(u => u.id === userId);
        if (userIndex === -1)
            return false;
    ...
    getMessagesByUsers(userId1, userId2) {
        return this.data.messages.filter(msg => (msg.senderId === userId1 && msg.recipientId === userId2) ||
            (msg.senderId === userId2 && msg.recipientId === userId1));
    }
    getMessagesByUser(userId) {
        return this.data.messages.filter(msg => msg.senderId === userId || msg.recipientId === userId);
    }
    getUnreadMessageCount(userId) {
        return this.data.messages.filter(msg => msg.recipientId === userId && !msg.read).length;
    }
