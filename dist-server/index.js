...
app.post('/api/reviews', authenticateToken, async (req, res) => {
    try {
        ...
        if (!tutorId || !rating)
            return res.status(400).json({ error: 'tutorId and rating are required' });
...
app.get('/api/admin/reviews', authenticateToken, requireRole(['ADMIN']), (req, res) => {
    ...
    const items = dataManager.getAllReviews(status);
    res.json({ items, total: items.length });
});
app.put('/api/admin/reviews/:reviewId/status', authenticateToken, requireRole(['ADMIN']), (req, res) => {
    try {
        ...
        if (review?.tutorId)
            dataManager.updateTutorRatingStats(review.tutorId);
...
app.post('/api/subjects/custom-request', authenticateToken, (req, res) => {
    try {
        ...
        if (!subjectName || !level)
            return res.status(400).json({ error: 'subjectName and level are required' });
...
function generateAISuggestions(profile) {
    ...
    appropriateSubjects.forEach((subject) => {
        if (!subjectsStudied.includes(subject) && !suggestions.find((s) => s.subject === subject)) {
            const subjectData = subjectsDatabase[subject];
            if (subjectData) {
                suggestions.push({
                    subject,
    ...
    const uniqueSuggestions = suggestions.filter((suggestion, index, self) => index === self.findIndex((s) => s.subject === suggestion.subject));
...
app.get('/api/tutors', async (req, res) => {
    try {
        const { q, priceMin, priceMax, ratingMin, page = '1', limit = '10' } = req.query;
        ...
        if (q && q.trim()) {
            ...
            tutors = tutors.filter((t) => (t.headline?.toLowerCase().includes(query)) ||
        ...
        if (priceMin) {
            ...
            if (!isNaN(min))
                tutors = tutors.filter((t) => (t.hourlyRateCents ?? 0) >= min);
        }
        if (priceMax) {
            const max = Number(priceMax);
            if (!isNaN(max))
                tutors = tutors.filter((t) => (t.hourlyRateCents ?? Infinity) <= max);
        }
        if (ratingMin) {
            const rmin = Number(ratingMin);
            if (!isNaN(rmin))
                tutors = tutors.filter((t) => (t.ratingAvg ?? 0) >= rmin);
        }
        // Pagination
...
app.post('/api/tutors', authenticateToken, async (req, res) => {
    try {
        const { headline, bio, hourlyRateCents, currency, yearsExperience, subjects, levels, slug, certifications, availabilityBlocks, inPersonLocation } = req.body;
...
app.get('/api/notifications', authenticateToken, async (req, res) => {
    try {
        ...
        const notifications = dataManager.getNotificationsByUserId(req.user?.userId || '', unreadOnly === 'true');
        res.json({
            success: true,
            notifications,
...
app.get('/api/tutors/bookings', authenticateToken, async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            errorLogger.logApiError('/api/tutors/bookings', 'GET', 404, new Error('User not found'), { userId: req.user?.userId });
            return res.status(404).json({ error: 'User not found' });
        }
        const tutor = dataManager.getTutorByUserId(user.id);
        ...
        if (!tutor) {
            ...
            errorLogger.logApiError('/api/tutors/bookings', 'GET', 404, new Error('Tutor profile not found'), { userId: req.user?.userId, userRole: user.role });
            return res.status(404).json({ error: 'Tutor profile not found' });
        }
...
app.get('/api/tutors/stats', authenticateToken, async (req, res) => {
    try {
        const user = dataManager.getUserById(req.user?.userId || '');
        if (!user) {
            errorLogger.logApiError('/api/tutors/stats', 'GET', 404, new Error('User not found'), { userId: req.user?.userId });
            return res.status(404).json({ error: 'User not found' });
        }
        const tutor = dataManager.getTutorByUserId(user.id);
        if (!tutor) {
            errorLogger.logApiError('/api/tutors/stats', 'GET', 404, new Error('Tutor profile not found'), { userId: req.user?.userId, userRole: user.role });
            return res.status(404).json({ error: 'Tutor profile not found' });
        }
        const tutorBookings = dataManager.getBookingsByTutorId(tutor.id);
...
function findTutorByIdOrSlug(identifier) {
    const tutors = dataManager.getAllTutors();
...
app.post('/api/reviews', authenticateToken, async (req, res) => {
    try {
        ...
        const existingReview = dataManager.getAllReviews().find(r => r.studentId === studentId && r.bookingId === bookingId);
        if (existingReview) {
...
app.post('/api/reviews/success-story', authenticateToken, async (req, res) => {
    try {
        ...
        const completedBookings = dataManager.getAllBookings().filter(b => b.studentId === studentId &&
            b.tutorId === tutorId &&
            b.status === 'COMPLETED');
...
app.post('/api/payments/create-dispute', authenticateToken, async (req, res) => {
    try {
        ...
        dataManager.addNotification({
            ...
            type: 'NEW_MESSAGE', // Using NEW_MESSAGE as closest match
